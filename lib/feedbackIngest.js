...
function ingestHumanFeedbackPlainText(fullText) {
  ...
  scanLines(reviewClean, "review_notes");

  /** Catch GENERAL RULE when export glued multiple clauses on one line or hid line breaks. */
  const seenRule = new Set(
    generalizable_feedback.map(function (g) {
      return sanitizeExportLine(g.feedback).toLowerCase();
    }),
  );
  const ruleRe = /\bGENERAL\s+RULE\s*:[^\r\n]+/gi;
  let rm;
  while ((rm = ruleRe.exec(reviewClean)) !== null) {
    const t = sanitizeExportLine(rm[0]);
    if (!t || !/^GENERAL\s+RULE\s*:/i.test(stripLeadingListMarkdown(t))) continue;
    const normalized = stripLeadingListMarkdown(t);
