 ...
 *
 * Marked feedback (preferred — anywhere in the Doc, including next to the relevant slide):
 * - FEEDBACK: …  → artifact-specific (case-insensitive marker; list prefixes OK).
 * - GENERAL RULE: … → generalizable / Learnings (same as before).
 *
 * Legacy: lines under === Review Notes === (and optional === Human feedback ===) still count
 * as artifact-specific when they do not use FEEDBACK:, so older workflows keep working.
 */
...
const { getLearningsDriveFolderId } = require("./env");
...
/**
 * Multiple FEEDBACK segments on one exported line (same pattern as GENERAL RULE).
 * @param {string} lineText
 * @returns {string[]}
 */
function extractFeedbackSegments(lineText) {
  const s = String(lineText || "");
  const indices = [];
  const re = /\bFEEDBACK\s*:/gi;
  let m;
  while ((m = re.exec(s)) !== null) indices.push(m.index);
  const parts = [];
  for (let i = 0; i < indices.length; i++) {
    const from = indices[i];
    const to = i + 1 < indices.length ? indices[i + 1] : s.length;
    parts.push(s.slice(from, to).trim());
  }
  return parts;
}

/**
 * Scan entire Doc plain text for FEEDBACK: and GENERAL RULE: markers (any section).
 */
function pushMarkedFeedbackFromFullDocument(fullText, seenArtifact, seenRule, artifact_specific_feedback, generalizable_feedback) {
  const lines = String(fullText || "").split(/\r?\n/);
  for (const line of lines) {
    const raw = sanitizeExportLine(line);
    if (!raw) continue;

    for (const seg of extractGeneralRuleSegments(raw)) {
    ...
    for (const seg of extractFeedbackSegments(raw)) {
      const normalized = stripLeadingListMarkdown(sanitizeExportLine(seg));
      if (!/\bFEEDBACK\s*:/i.test(normalized)) continue;
      const after = normalized.replace(/^\s*FEEDBACK\s*:\s*/i, "").trim();
      if (!after) continue;
      const key = normalized.toLowerCase();
...
function pushArtifactLinesFromBlob(blob, sourceLabel, artifact_specific_feedback) {
  ...
  for (const line of lines) {
    ...
    // Inline FEEDBACK: is collected from the full-document scan; skip to avoid duplicates.
    if (/\bFEEDBACK\s*:/i.test(raw)) continue;
    artifact_specific_feedback.push({
      feedback: raw,
      source: sourceLabel,
      applies_to: "current_artifact",
...
function ingestHumanFeedbackPlainText(fullText) {
  ...
  pushMarkedFeedbackFromFullDocument(
    fullText,
    seenArtifact,
    seenRule,
...
async function applyGeneralRulesFromFeedback(fb, ctx) {
  ...
  let judgmentEntriesAdded = 0;
  let projectRulesBulletsAdded = 0;
  let lastJudgmentFileId = "";
  let lastRulesFileId = "";
  ...
  for (const item of fb.generalizable_feedback || []) {
    ...
    const j = await appendJudgmentEntry({
    ...
    lastJudgmentFileId = j.fileId || lastJudgmentFileId;
    judgmentEntriesAdded += 1;

    const merge = await mergeProjectRulesBullet(sectionSlug, suggested);
    lastRulesFileId = merge.fileId || lastRulesFileId;
    if (merge.updated) projectRulesBulletsAdded += 1;

    item.inferred_task_type = inferred;
    item.suggested_rule = suggested;
  }

  const rulesUpdated = judgmentEntriesAdded > 0 || projectRulesBulletsAdded > 0;

  return {
    rulesUpdated,
    judgmentEntriesAdded,
    projectRulesBulletsAdded,
    judgmentLogFileId: lastJudgmentFileId,
    projectRulesFileId: lastRulesFileId,
    learningsFolderConfigured: Boolean(getLearningsDriveFolderId()),
  };
}
