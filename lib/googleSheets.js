...
/**
 * @param {string} [spreadsheetId] - defaults to SPREADSHEET_ID from env
 */
async function getSheetsForSpreadsheet(spreadsheetId) {
  const id = String(spreadsheetId || SPREADSHEET_ID || "").trim();
  const creds = getServiceAccountCredentials();
  if (!creds || !id) {
    const err = new Error("NOT_CONFIGURED");
    err.code = "SHEETS_NOT_CONFIGURED";
