...
/**
 * Ensure row 1 has Week 6 QA / Enforcer headers after the execution block.
 * @returns {Promise<{ updated: boolean, message?: string }>}
 */
async function ensureMasterQaSchema() {
  const { sheets, spreadsheetId } = await getClient();
  const tab = TABS.MASTER;
  const end = masterEndColumnLetter();
  const range = `${sheetRangePrefix(tab)}!A1:${end}1`;
  ...
  let changed = false;
  const warnings = [];
  for (let j = 0; j < MASTER_QA_HEADER_LABELS.length; j++) {
    const i = MASTER_QA_START_INDEX + j;
    const expected = MASTER_QA_HEADER_LABELS[j];
    const cur = row[i];
    if (!cur) {
      row[i] = expected;
      changed = true;
    } else if (cur !== expected) {
      warnings.push(`Column ${i + 1}: expected QA header "${expected}", found "${cur}" (left unchanged).`);
    }
  }
  if (changed) {
    await sheets.spreadsheets.values.update({
  ...
  if (warnings.length) {
    console.warn("[qa schema]", warnings.join(" "));
  }
  return {
    updated: changed,
    message: warnings.length ? warnings.join(" ") : undefined,
