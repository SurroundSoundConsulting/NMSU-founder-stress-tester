...
async function runHumanFeedbackRevision(opts) {
  ...
  const rootFolder = getExecutionDriveFolderId();
  const taskFolderId = await ensureTaskArtifactFolder(rootFolder, working[IDX.task_id] || String(sheetRow));
  const nextLabel = "Human-feedback v" + humanFeedbackVersionNum;
  ...
  return {
    ...
    doc_url: doc.url,
    full_plain_text,
    human_feedback_version: humanFeedbackVersionNum,
  };
}
 ...
 *
 * @param {{ apiKey: string, model: string, working: string[], docUrl: string, fullDocPlainText: string, fb: object, stopReason?: string, deltaBaselineFinalScore?: number }} opts
 */
async function runPostHumanReviewRescore(opts) {
  ...
  let baselineNum =
    deltaBaselineFinalScore != null && Number.isFinite(deltaBaselineFinalScore)
      ? deltaBaselineFinalScore
      : Number(String(working[QDX.final_enforcer_score] || "").trim());
  const delta =
    Number.isFinite(baselineNum) && Number.isFinite(newScore)
      ? Math.round(newScore - baselineNum)
      : "";

  working[QDX.universal_score] = String(combOut.universal_score);
  working[QDX.task_type_score] = String(combOut.task_type_score);
  working[QDX.task_specific_score] = String(combOut.task_specific_score);
  working[QDX.final_enforcer_score] = String(combOut.overall_score);
  working[QDX.ready_for_production] = combOut.production_ready;
  working[QDX.stop_reason] = String(opts.stopReason || "post_human_review").trim() || "post_human_review";
  if (delta !== "") {
