...
async function runPostHumanReviewRescore(opts) {
  ...
  working[QDX.ready_for_human_review] = newScore >= getQualityThreshold() ? "Yes" : "No";
  const thr = getQualityThreshold();
  let sr = String(opts.stopReason || "post_human_review").trim() || "post_human_review";
  if (newScore < thr) {
    sr +=
      " | Not_ready: score " +
      newScore +
      " < threshold " +
      thr +
      " — refine FEEDBACK/GENERAL_RULE and re-ingest, or lower ENFORCER_QUALITY_THRESHOLD";
  }
  working[QDX.stop_reason] = sr;
  if (delta !== "") {
  ...
  return {
    ...
    ready_for_human_review: combOut.ready_for_human_review,
    doc_url: docUrl,
  };
}
