...
const { MASTER_COLUMNS, formatUniversalTaskArtifactCell } = require("./sheetSchema");
const {
...
const {
  ...
  fetchFeedbackLinkedGoogleDocsPlainText,
...
/** Compare Doc URLs by file id when possible so trivial query-string differences still match. */
function googleDocUrlsSameDoc(a, b) {
  const ida = parseGoogleDocFileId(String(a || ""));
  const idb = parseGoogleDocFileId(String(b || ""));
  if (ida && idb) return ida === idb;
  return String(a || "").trim() === String(b || "").trim();
}

/** Sheet display for "Ready for Production" column. */
function formatProductionReadyCell(score, threshold) {
  const thr = Number(threshold);
  const s = Number(score);
  if (!Number.isFinite(s)) return "No";
  if (!Number.isFinite(thr)) {
    return s >= 90 ? "Yes" : "No - score below 90";
  }
  if (s >= thr) return "Yes";
  return "No - score below " + thr;
}
...
const QDX = {
  ...
  universal_task_artifact_score: colQ("universal_task_artifact_score"),
  ...
  ready_for_production: colQ("ready_for_production"),
...
/** Sheet Stop Reason when feedback path rescored without running the human-feedback revision loop. */
const STOP_REASON_RESCORE_ONLY_NO_REVISION = "Rescoring only - no revision";
...
function listEnforcerCandidates(dataRows, taskIdFilter, force) {
  const out = [];
  const want = taskIdFilter != null ? normalizeTaskId(taskIdFilter) : "";
  for (let i = 0; i < dataRows.length; i++) {
    ...
    if (!outLink) continue;
    if (want && normalizeTaskId(row[IDX.task_id]) !== want) continue;
...
function applyQaToRow(row, patch) {
  ...
  set(QDX.universal_task_artifact_score, patch.universal_task_artifact_score);
  ...
  set(QDX.ready_for_production, patch.ready_for_production);
...
async function runEnforcerOnce(opts) {
  ...
  try {
    ...
    while (!stopReason && revisionAttemptsUsed < maxAttempts) {
      ...
      lastDelta = formatRevisionDeltaEnforcerStep(improvementDelta);
    ...
    const readyStr = formatProductionReadyCell(summary.final_score, qualityThreshold);
    summary.ready_for_production = readyStr;
    ...
    if (ingestFeedback && feedbackId) {
      try {
        ...
        rulesUpdated =
          applied.projectRulesBulletsAdded > 0
            ? "Yes"
            : applied.judgmentEntriesAdded > 0
              ? "Yes (judgment log only)"
              : "No";
      } catch (e) {
    ...
    working = applyQaToRow(working, {
      qa_status: "complete",
      universal_task_artifact_score: formatUniversalTaskArtifactCell(
        evalForSheet.combined.universal_score,
        evalForSheet.combined.task_type_score,
        evalForSheet.combined.task_specific_score,
      ),
...
/** Google Sheets USER_ENTERED interprets a leading "+" as a formula; wrap signed deltas in parentheses. */
function sheetSafeSignedDelta(d) {
  const x = Math.round(Number(d));
...
/** Sheet cell after feedback rescore: compares new final score to baseline (fair prior best or prior sheet final). */
function formatRevisionDeltaFeedback(newScore, baselineScore, basis) {
  const n = Number(newScore);
  const b = Number(baselineScore);
  if (!Number.isFinite(n) || !Number.isFinite(b)) return "";
  const d = Math.round(n - b);
  const label =
    basis === "fair_prior_best"
      ? "fair prior best"
      : basis === "sheet_prior_final"
        ? "prior sheet final"
        : "baseline";
  return sheetSafeSignedDelta(d) + " (" + label + ": " + b + " -> " + n + ")";
}

/** Main Enforcer loop: delta is improvement from one scored revision to the next within the same run. */
function formatRevisionDeltaEnforcerStep(improvementDelta) {
  const parens = sheetSafeSignedDelta(improvementDelta);
  if (!parens) return "";
  return parens + " (step vs prior pass in Enforcer)";
}
...
async function augmentContextWithFeedbackDocLinks(contextDocText, fbSummary, clientEmail) {
  const fb = String(fbSummary || "").trim();
  const base = String(contextDocText || "").trim();
  if (!fb) return base;
  const perFile = getExecutionContextDocMaxCharsPerFile();
  const totalBudget = getExecutionContextDocMaxCharsTotal();
  const linkedBudget = Math.min(24000, Math.floor(totalBudget / 2));
  const { text: fbLinked } = await fetchFeedbackLinkedGoogleDocsPlainText(fb, {
    perFileMaxChars: perFile,
    totalMaxChars: linkedBudget,
    maxFiles: 3,
    clientEmail,
  });
  const tail = String(fbLinked || "").trim();
  if (!tail) return base;
  return (
    base +
    "\n\n=== Google Docs linked from reviewer feedback (truncated) ===\n\n" +
    tail
  );
}

/**
 * Human-feedback revision loop: same stop rules as the main Enforcer (threshold, min improvement delta,
 * score decrease, max attempts). Creates one Google Doc per revision attempt.
 *
 * @param {{ apiKey: string, model: string, working: string[], fb: object, sourceFullPlainText: string, sheetRow: number, humanFeedbackVersionNum?: number }} opts
 */
async function runHumanFeedbackRevision(opts) {
  const { apiKey, model, working, fb, sourceFullPlainText, sheetRow } = opts;
  const humanFeedbackVersionNum =
    Number.isFinite(opts.humanFeedbackVersionNum) && opts.humanFeedbackVersionNum > 0
      ? Math.floor(opts.humanFeedbackVersionNum)
      : 1;
  const qualityThreshold = getQualityThreshold();
  const minDelta = getMinImprovementDelta();
  const maxAttempts = getMaxRevisionAttempts();
  ...
  const mergedContext = await augmentContextWithFeedbackDocLinks(contextDocText, fbSummary, clientEmail);
  ...
  async function evaluateFeedbackArtifact(text) {
    const u = await evaluateUniversalQA(apiKey, model, text, taskBundleForEval);
    const tt = await generateTaskTypeRubric(apiKey, model, taskBundleForEval, executionPrompt, text);
    const ts = await generateTaskSpecificCriteria(
      apiKey,
      model,
      taskBundleForEval,
    ...
    return {
      ...
      taskSpecific: ts,
      combined: comb,
      score: comb.overall_score,
    };
  }

  let currentText = artifactText;
  let lastEval = await evaluateFeedbackArtifact(currentText);
  let stopReason = "";
  let revisionAttemptsUsed = 0;
  let prevScore = lastEval.score;
  let lastDocUrl = "";
  ...
  while (!stopReason && revisionAttemptsUsed < maxAttempts) {
    revisionAttemptsUsed += 1;
    const versionLabel = "Human-feedback v" + (humanFeedbackVersionNum + revisionAttemptsUsed - 1);

    const revisedText = await reviseArtifact(apiKey, model, {
      taskBundle: taskBundleBase,
      executionPrompt,
      contextDocText: mergedContext,
      currentArtifact: currentText,
      universalJson: lastEval.universal,
      taskTypeRubricJson: lastEval.taskType,
      taskSpecificJson: lastEval.taskSpecific,
    ...
    lastFullPlain = await exportGoogleDocPlainTextForFeedback(newId);

    lastEval = await evaluateFeedbackArtifact(currentText);
    const newScore = lastEval.score;
    const improvementDelta = newScore - prevScore;

    revision_history.push({
      attempt: revisionAttemptsUsed,
  ...
  return {
    ok: true,
    doc_url: lastDocUrl,
    full_plain_text: lastFullPlain,
    human_feedback_version: humanFeedbackVersionNum + revisionAttemptsUsed - 1,
    revision_attempts: revisionAttemptsUsed,
...
/**
 * Full Enforcer scoring with the same task row + human-feedback-augmented bundle (no sheet mutation).
 * Use this to compare two revision Docs fairly under identical criteria.
 *
 * @param {{ apiKey: string, model: string, working: string[], fullDocPlainText: string, fb: object }} opts
 */
async function computePostHumanFeedbackScores(opts) {
  const { apiKey, model, working, fullDocPlainText, fb } = opts;
  const qualityThreshold = getQualityThreshold();
  ...
  const taskBundle = appendFeedbackToTaskBundle(buildTaskBundleText(working, padRow, IDX), fbSummary);
  ...
  return {
    ...
    production_ready: formatProductionReadyCell(newScore, qualityThreshold),
  };
}
 ...
 *
 * @param {{ apiKey: string, model: string, working: string[], docUrl: string, fullDocPlainText: string, fb: object, stopReason?: string, deltaBaselineFinalScore?: number }} opts — stopReason should match main Enforcer vocabulary when feedback revision ran (e.g. quality_threshold_met); use "Rescoring only - no revision" when only rescoring without that loop.
 */
async function runPostHumanReviewRescore(opts) {
  const { working, docUrl, deltaBaselineFinalScore } = opts;

  const sheetPriorFinalRaw = Number(String(working[QDX.final_enforcer_score] || "").trim());
  const sheetPriorFinal = Number.isFinite(sheetPriorFinalRaw) ? sheetPriorFinalRaw : NaN;

  const combOut = await computePostHumanFeedbackScores(opts);
  const newScore = combOut.overall_score;

  let baselineNum;
  /** @type {"fair_prior_best"|"sheet_prior_final"} */
  let deltaBasis;
  if (deltaBaselineFinalScore != null && Number.isFinite(deltaBaselineFinalScore)) {
    baselineNum = deltaBaselineFinalScore;
    deltaBasis = "fair_prior_best";
  } else if (Number.isFinite(sheetPriorFinal)) {
    baselineNum = sheetPriorFinal;
    deltaBasis = "sheet_prior_final";
  } else {
    baselineNum = NaN;
    deltaBasis = "sheet_prior_final";
  }
  const delta =
    Number.isFinite(baselineNum) && Number.isFinite(newScore)
      ? Math.round(newScore - baselineNum)
      : "";

  working[QDX.universal_task_artifact_score] = formatUniversalTaskArtifactCell(
    combOut.universal_score,
    combOut.task_type_score,
    combOut.task_specific_score,
  );
  working[QDX.final_enforcer_score] = String(combOut.overall_score);
  working[QDX.ready_for_production] = combOut.production_ready;
  working[QDX.stop_reason] =
    String(opts.stopReason || STOP_REASON_RESCORE_ONLY_NO_REVISION).trim() ||
    STOP_REASON_RESCORE_ONLY_NO_REVISION;
  if (Number.isFinite(baselineNum) && Number.isFinite(newScore)) {
    working[QDX.revision_delta] = formatRevisionDeltaFeedback(newScore, baselineNum, deltaBasis);
  }
  ...
  return {
    ok: true,
    overall_score: newScore,
    universal_score: combOut.universal_score,
    task_type_score: combOut.task_type_score,
    task_specific_score: combOut.task_specific_score,
    delta_vs_prior_final: delta,
    delta_basis: deltaBasis,
    ready_for_production: combOut.production_ready,
    doc_url: docUrl,
  };
}
...
async function runFeedbackIngestOnce(opts) {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    const row = padRow(dataRows[i]);
    if (!taskIdsEqual(row[IDX.task_id], want)) continue;
    sheetRow = i + 2;
    working = row;
    break;
  }
  ...
  working[QDX.general_rules_updated] =
    applied.projectRulesBulletsAdded > 0
      ? "Yes"
      : applied.judgmentEntriesAdded > 0
        ? "Yes (judgment log only)"
  ...
  if (shouldRevise) {
    try {
      revisionSummary = await runHumanFeedbackRevision({
        ...
        sheetRow,
        humanFeedbackVersionNum: revisionCountBefore + 1,
      });
      if (revisionSummary.ok) {
        ...
        const attempts =
          Number.isFinite(revisionSummary.revision_attempts) && revisionSummary.revision_attempts > 0
            ? revisionSummary.revision_attempts
            : 1;
        working[QDX.revision_count] = String(revisionCountBefore + attempts);
      }
    } catch (err) {
  ...
  /** @type {object | null} */
  let bestRevisionDecision = null;
  if (shouldRescore) {
    try {
      let stopReason =
        revisionSummary?.ok && revisionSummary.loop_stop_reason
          ? revisionSummary.loop_stop_reason
          : STOP_REASON_RESCORE_ONLY_NO_REVISION;

      let deltaBaselineFinalScore = undefined;
      let previousBestFairScore = undefined;

      if (
        revisionSummary?.ok &&
        bestUrlBefore &&
        !googleDocUrlsSameDoc(bestUrlBefore, docUrlForRescore)
      ) {
        const oldBestId = parseGoogleDocFileId(bestUrlBefore);
        if (oldBestId) {
          const oldPlain = await exportGoogleDocPlainTextForFeedback(oldBestId);
          const oldFair = await computePostHumanFeedbackScores({
            apiKey,
            model,
            working,
            fullDocPlainText: oldPlain,
            fb,
          });
          previousBestFairScore = oldFair.overall_score;
          deltaBaselineFinalScore = previousBestFairScore;
        }
      }
      ...
      if (revisionSummary?.ok && rescoreSummary.ok) {
        ...
        if (
          bestUrlBefore &&
          !googleDocUrlsSameDoc(bestUrlBefore, docUrlForRescore) &&
          previousBestFairScore != null &&
          Number.isFinite(previousBestFairScore)
        ) {
          const oldWins = previousBestFairScore > newScore;
          working[QDX.best_revision_url] = oldWins ? bestUrlBefore : docUrlForRescore;
          bestRevisionDecision = {
            compared_previous_best_under_same_feedback_rubric: true,
            previous_best_url: bestUrlBefore,
            previous_best_fair_score: previousBestFairScore,
            new_revision_score: newScore,
            picked_best_url: oldWins ? bestUrlBefore : docUrlForRescore,
          };
        } else {
          working[QDX.best_revision_url] = docUrlForRescore;
          bestRevisionDecision = {
            compared_previous_best_under_same_feedback_rubric: false,
            picked_best_url: docUrlForRescore,
          };
        }
      }
    } catch (err) {
  ...
  return {
    ...
    learnings: {
      judgment_entries_added: applied.judgmentEntriesAdded,
      project_rules_bullets_added: applied.projectRulesBulletsAdded,
      judgment_log_file_id: applied.judgmentLogFileId,
      project_rules_file_id: applied.projectRulesFileId,
      learnings_folder_configured: applied.learningsFolderConfigured,
    },
    revision: revisionSummary,
    rescore: rescoreSummary,
    best_revision_decision: bestRevisionDecision,
  };
}
...
module.exports = {
  ...
  computePostHumanFeedbackScores,
  taskIdsEqual,
