...
async function runEnforcerOnce(opts) {
  ...
  try {
    ...
    let humanCaptured = "";
    let rulesUpdated = "";
    const bestId = parseGoogleDocFileId(bestUrl);
    if (ingestFeedback && bestId) {
      try {
        const fbText = await exportGoogleDocPlainText(bestId);
...
/**
 * Ingest feedback from Best Revision URL (fallback: Execution Output Link) for a task row.
