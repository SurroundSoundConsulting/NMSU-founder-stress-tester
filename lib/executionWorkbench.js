...
async function openAiChatJson(apiKey, model, system, user) {
  ...
  try {
    ...
    const err = new Error("Could not parse classifier JSON: " + String(e && e.message));
    err.code = "CLASSIFY_PARSE";
    throw err;
  }
}

async function openAiChatText(apiKey, model, system, user) {
  const response = await fetch(OPENAI_URL, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
