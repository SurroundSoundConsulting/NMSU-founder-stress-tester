...
/**
 * Scan arbitrary text (e.g. FEEDBACK: lines) for Google Drive file ids / URLs in order of appearance.
 * @param {string} text
 * @param {number} maxFiles
 * @returns {string[]}
 */
function extractGoogleDriveFileIdsFromText(text, maxFiles) {
  const cap = Number.isFinite(maxFiles) && maxFiles > 0 ? Math.min(maxFiles, 10) : 5;
  const s = String(text || "");
  const seen = new Set();
  const ids = [];
...
/**
 * Export Google Docs plain text for a list of file ids (shared helper).
 *
 * @param {string[]} ids
 * @param {{ perFileMaxChars?: number, totalMaxChars?: number, clientEmail?: string }} [options]
 * @returns {Promise<{ text: string, errors: { fileId: string, message: string }[] }>}
 */
async function fetchGoogleDocsPlainTextByIds(ids, options) {
...
/**
 * Fetch Google Docs referenced by URLs/ids inside reviewer feedback text (FEEDBACK / GENERAL RULE lines).
 *
 * @param {string} feedbackPlainText
 * @param {{ perFileMaxChars?: number, totalMaxChars?: number, maxFiles?: number, clientEmail?: string }} [options]
 * @returns {Promise<{ text: string, errors: { fileId: string, message: string }[] }>}
 */
async function fetchFeedbackLinkedGoogleDocsPlainText(feedbackPlainText, options) {
  const maxFiles = options?.maxFiles ?? 3;
  const ids = extractGoogleDriveFileIdsFromText(feedbackPlainText, maxFiles);
  return fetchGoogleDocsPlainTextByIds(ids, options);
}
...
/**
 * Enforcer revision Docs use === Artifact ===; Week 5 execution Docs use LLM Output section.
 * @param {string} fullText
 */
function extractArtifactSectionFromDoc(fullText) {
  const text = String(fullText || "");
